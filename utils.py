...
def get_statistics_data():
    ...
    # 1. 消息类型分布
    msg_types = session.query(Message.type, func.count(Message.id)).group_by(Message.type).all()
    stats['message_types'] = {t[0]: t[1] for t in msg_types}
    
    # 2. 用户活跃度 - 发送消息最多的前10名用户
    top_users = session.query(Message.from_id, func.count(Message.id).label('count')) \
    ...
    # 4. 时间模式分析
    # 按小时统计
    hour_stats = session.query(func.extract('hour', Message.date).label('hour'), 
    ...
    # 按星期几统计
    weekday_stats = session.query(func.extract('dow', Message.date).label('weekday'), 
                                 func.count(Message.id).label('count')) \
                          .group_by('weekday') \
    ...
    # 按月份统计
    month_stats = session.query(func.extract('month', Message.date).label('month'), 
                               func.count(Message.id).label('count')) \
                        .group_by('month') \
                        .order_by('month').all()
    ...
    for start, end, label in length_ranges:
        if end == float('inf'):
            count = session.query(func.count(Message.id)) \
                          .filter(func.length(Message.text) > start).scalar()
        else:
            count = session.query(func.count(Message.id)) \
                          .filter(func.length(Message.text) >= start) \
                          .filter(func.length(Message.text) <= end).scalar()
        length_stats[label] = count
    
    stats['message_length'] = length_stats
    
    # 6. 总体统计
    stats['total_messages'] = session.query(func.count(Message.id)).scalar()
    ...
    stats['recent_messages'] = session.query(func.count(Message.id)) \
                                    .filter(Message.date >= recent_date).scalar()
